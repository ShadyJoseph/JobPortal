...
const EditJob = () => {
  ...
  // Fetch the existing job details when the component mounts
  useEffect(() => {
    const fetchJobDetails = async () => {
      try {
        const response = await api.get(`/jobs/${jobId}`);
        setFormData(response.data); // Set the form data to the job details
  ...
  return (
    <div className="container mx-auto p-6 max-w-xl">
      <h1 className="text-3xl font-bold text-center mb-6 text-gray-800">Edit Job</h1>
      ...
      <form onSubmit={handleSubmit} className="bg-white shadow-md rounded-lg p-8 mb-4">
        ...
        <div className="flex items-center justify-between">
          ...
          >
            {loading ? (
              <>
                <Loader height="25" width="25" /> Updating...
              </>
            ) : (
              'Update Job'
            )}
